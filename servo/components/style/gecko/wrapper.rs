...
:
:
nsIContent_nsContentSlots_sNonOwningExtendedSlotsFlag
;
(
e_slots
as
...
fn
is_root_of_native_anonymous_subtree
(
&
self
)
...
ElementSelectorFlags
{
slow_selector_flags_from_node_selector_flags
(
self
.
as_node
...
:
:
MAY_HAVE_TREE_COUNTING_FUNCTION
)
{
gecko_flags
|
...
:
:
Gecko_Element_GetSubtreeBloomFilter
(
self
.
0
...
:
:
Gecko_NoteHighlightPseudoStyleInvalidated
(
doc
)
;
...
:
:
Gecko_GetImplementedPseudoIdentifier
(
self
.
0
...
:
:
Gecko_GetImplementedPseudoType
(
self
.
0
...
fn
synthesize_view_transition_dynamic_rules
<
V
>
(
...
:
:
Gecko_GetViewTransitionDynamicRule
(
self
.
0
...
:
:
Gecko_HasActiveViewTransitionTypes
(
self
.
as_node
