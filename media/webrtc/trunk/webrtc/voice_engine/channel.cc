...
SetLastError
(
VE_SOCKET_TRANSPORT_MODULE_ERROR
kTraceWarning
"
Channel
:
...
random
offset
_sendTelephoneEventPayloadType
(
106
)
_playoutTimeStampRTP
...
:
:
ACMToExternalCodecRepresentation
(
codec
codecCopy
)
...
Enable
(
WEBRTC_VOICE_ENGINE_RX_HP_DEFAULT_STATE
)
!
=
0
...
Enable
(
WEBRTC_VOICE_ENGINE_RX_NS_DEFAULT_STATE
)
!
=
0
...
WEBRTC_VOICE_ENGINE_RX_AGC_DEFAULT_MODE
)
!
=
0
...
Enable
(
WEBRTC_VOICE_ENGINE_RX_AGC_DEFAULT_STATE
)
!
=
0
...
SetLastError
(
VE_BINDING_SOCKET_TO_LOCAL_ADDRESS_FAILED
kTraceError
"
SetLocalReceiver
(
...
:
:
ExternalToACMCodecRepresentation
(
acmCodec
codec
)
...
rateBps
>
kVoiceEngineMaxIsacInitTargetRateBpsWb
)
)
{
_engineStatisticsPtr
...
rateBps
<
kVoiceEngineMinIsacInitTargetRateBpsSwb
)
|
|
(
rateBps
>
kVoiceEngineMaxIsacInitTargetRateBpsSwb
)
)
{
_engineStatisticsPtr
...
rateBps
<
kVoiceEngineMinIsacMaxRateBpsWb
)
|
|
(
...
rateBps
<
kVoiceEngineMinIsacMaxRateBpsSwb
)
|
|
(
rateBps
>
kVoiceEngineMaxIsacMaxRateBpsSwb
)
)
{
_engineStatisticsPtr
...
sizeBytes
<
kVoiceEngineMinIsacMaxPayloadSizeBytesSwb
)
|
|
(
...
cipherKeyLength
<
kVoiceEngineMinSrtpEncryptLength
|
|
cipherKeyLength
>
kVoiceEngineMaxSrtpEncryptLength
)
)
|
|
...
authKeyLength
>
kVoiceEngineMaxSrtpKeyAuthNullLength
|
|
authTagLength
>
kVoiceEngineMaxSrtpTagAuthNullLength
)
)
)
{
