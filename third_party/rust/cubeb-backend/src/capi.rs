...
:
:
capi_get_preferred_sample_rate
:
:
<
ctx
...
:
:
capi_get_supported_input_processing_params
:
:
<
ctx
...
:
:
capi_device_collection_destroy
:
:
<
ctx
...
:
:
capi_stream_get_current_device
:
:
<
stm
...
:
:
capi_stream_register_device_changed_callback
:
:
<
stm
